...
async def root():
    response = {
        ...
        "versions": {"python": sys.version, "tensorflow": tf.__version__},
    }
...
async def upload_model(model: UploadFile = File(...)):
    ...
    if model.filename.endswith(".zip"):
        ...
        shutil.rmtree("./model")
        mkdir("./model")
        with io.BytesIO(await model.read()) as tmp_stream, zipfile.ZipFile(
            tmp_stream, "r"
        ) as zip_ref:
        ...
        raise HTTPException(
            status_code=400,
            detail="Invalid file type. Only .zip and .onnx files are accepted.",
        )
